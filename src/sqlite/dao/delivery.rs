...
//                debug!("insert carrier to db for id: {:?} successful", carrier.id);
    ...
    fn delete(carrier_id: u128) -> Result<(), NatureError> {
    ...
    fn move_to_error<T: Sized + Serialize + Debug>(_err: CarryError<T>) -> Result<(), NatureError> {
        unimplemented!()
    }

    fn update_execute_time(_id: u128, _new_time: i64) -> Result<(), NatureError> {
        unimplemented!()
    }

    fn get<T: Sized + Serialize + Debug>(_id: u128) -> Result<Carrier<T>, NatureError> {
        unimplemented!()
    }
}
